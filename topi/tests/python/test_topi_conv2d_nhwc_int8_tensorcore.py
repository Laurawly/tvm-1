...
def verify_conv2d_nhwc(batch, in_channel, in_size, num_filter, kernel, stride,
    ...
    A = te.placeholder((batch, in_height, in_width, in_channel), name='A', dtype='int')
    W = te.placeholder((kernel, kernel, in_channel, num_filter), name='W', dtype='int')
    bias = te.placeholder((1, 1, 1, num_filter), name='bias', dtype='int')
    ...
    def check_device(device):
        ...
        with tvm.target.create(device):
            ...
            C = fcompute(A, W, stride, padding, dilation, 'int')
            if add_bias:
                C = topi.add(C, bias)
            if add_relu:
                C = topi.nn.relu(C)
